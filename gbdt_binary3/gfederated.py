...
from gsites import sequential_local_je1
from gsites import sequential_local_jl1
from InData.v_input import v_DataSet
from InData.t_input import t_DataSet
...
for i in range(1, 100):
    ...
    fed_t = t_DataSet(2, train_dataset,head_dataset) 
    # print(fed_t.label)
    # 第一个参数表示读取data instance从第几列开始（因为有重复项），第二个参数是读取的数据集
    #第三个参数是读取特征名称
    fed_v = v_DataSet(1, valid_dataset,head_dataset)
    
    """
    train_url = "/home/zhangfeifei/boosting/gbdt_binary/data/adult_" + str(node_id) + ".csv"
    valid_url = "/home/zhangfeifei/boosting/gbdt_binary/data/adult_" + str(node_id) + "_.csv"
    train_dataset = pd.read_csv(train_url)
    valid_dataset = pd.read_csv(valid_url)
    train_dataset = np.array(train_dataset)
    valid_dataset = np.array(valid_dataset)
    ...
    # print(train_dataset.shape)
    fed_t = t_DataSet(0, train_dataset,head_dataset) 
    
    fed_v = t_DataSet(0, valid_dataset,head_dataset)
    # print(fed_v.label)
    # print(fed_t.dsize)
    """
...
for j in range(1, node_local.tao + 1):
    ...
    """
    if j == 1:
        f, node_list = modified_local_je1(is_selection, node_list, node_local, max_iter,
                    sample_rate, learn_rate, max_depth, node_local.loss, num_choice)
    else:
        node_list = modified_local_jl1(is_selection, node_list, node_local, max_iter,
                    sample_rate, learn_rate, max_depth, f, node_local.loss, num_choice)
    """
    if j == 1:
        f, node_list = sequential_local_je1(is_selection, node_list, node_local, max_iter,
                    sample_rate, learn_rate, max_depth, node_local.loss, num_choice)
    else:
        node_list = sequential_local_jl1(is_selection, node_list, node_local, max_iter,
                    sample_rate, learn_rate, max_depth, f, node_local.loss, num_choice)
