...
class Local_Node:
    ...
    def concatenate_temp(self, tree, treelf):
        self.temp_trees.append(tree)

        self.temp_treelfs = np.concatenate([self.temp_treelfs, [treelf]])
        return self.temp_trees, self.temp_treelfs
    ...
    def clean_base_weight_temp(self):
        self.temp_trees = []
        self.temp_treelfs = np.array([])
        # self.M = None
        return self.temp_trees, self.temp_treelfs

    def concatenate_model_local(self, tem_tree, tem_treelf):
        print('!!!!!!!Concat', self.cur_treelfs.shape)
        self.cur_trees.append(tem_tree)
        self.cur_treelfs = np.concatenate([self.cur_treelfs, [tem_treelf]])
        return self.cur_trees, self.cur_treelfs
    
    def batch_concatenate_model_local(self, tem_trees, tem_treelfs):
        print('!!!!!!!Concat', self.cur_treelfs.shape)
        for i in range(len(tem_trees)):
            self.cur_trees.append(tem_trees[i])
            self.cur_treelfs = np.concatenate([self.cur_treelfs, [tem_treelfs[i]]])
