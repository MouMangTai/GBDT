...
class BinomialDeviance(ClassificationLossFunction):
    ...
    # 新增: 更新h的值
    def compute_h(self, dataset, subset, f):
        h = {}
        for id in subset:
            y_i = dataset.get_instance(id)['label']
            try:
                y_i = int(y_i)
                ans1 = (4 * pow(y_i, 2) * exp((-2) * y_i * f[id])) / (log(2) * (exp((-2) * y_i * f[id])) + 1)
                ans2 = (4 * pow(y_i, 2) * exp((-4) * y_i * f[id])) / (
    ...
    # 新增: 算法2 的实现，计算出G和H
    def computeGH(self, nodes, train_node, f, Pre_treelfs, subset, dataset, learn_rate):
        # 算法2开始

        G = [[0.0 for i in range(len(train_node.origin_dataset) + 1)] for i in range(len(nodes) + 1)]
        H = [[0.0 for i in range(len(train_node.origin_dataset) + 1)] for i in range(len(nodes) + 1)]

        # 遍历所有的数据集
        for node in nodes:

            cur_dataset = node.fed_train
            cur_subset = cur_dataset.get_instances_idset()

            # 跳过当前node
            if train_node.id == node.id:
                continue
            # Update the gradients of instances in I,更新g和h
            if Pre_treelfs is not None:
                f = self.update_fset_value(f, Pre_treelfs, cur_subset, cur_dataset, learn_rate, label=None)

            residual = self.compute_residual(cur_dataset, cur_subset, f)
        ...
        if Pre_treelfs is not None:
            f = self.update_fset_value(f, Pre_treelfs, subset, dataset, learn_rate, label=None)
        residual = self.compute_residual(dataset, subset, f)
        h = self.compute_h(dataset, subset, f)

        # finallyG finallyH
        fG = {}
        fH = {}

        for x in range(len(train_node.origin_dataset)):
            fG[x] = 0
            fH[x] = 0
    ...
    def update_f_value(self, f, tree, leaf_nodes, subset, dataset, learn_rate, label=None):
        ...
        for node in leaf_nodes:
            for id in node.get_idset():
                id = int(id)
                f[id] += float(learn_rate) * node.get_predict_value()
        # 更新OOB的样本
        for id in data_idset - subset:
            f[id] += float(learn_rate) * tree.get_predict_value(dataset.get_instance(id))
        return f

    def update_fset_value(self, temp_f, treelfs, subset, dataset, learn_rate, label=None):

        for i in range(len(treelfs)):
            treelf = treelfs[i]
            tree = treelf.get_tree()
            lf = treelf.get_lf()

            leaf_nodes = lf.get_leafnodes()
            temp_f = self.initialize(temp_f, dataset)
            temp_f = self.update_f_value(temp_f, tree, leaf_nodes, subset, dataset, learn_rate, label=None)
            # print(temp_f)
            if i == 0:
                aver_f = temp_f
            else:
                for key, var in aver_f.items():
                    aver_f[key] += temp_f[key]
        for key, var in aver_f.items():
            aver_f[key] = aver_f[key] / len(treelfs)
        return aver_f
